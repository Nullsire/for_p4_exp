...
def generate_receiver_script(args, config, filename="run_receiver.sh"):
    ...
    lines.append("  # Use ss filter to match destination ports (server-side local ports)")
    lines.append("  # This avoids double-counting that occurs with grep on both local and remote addresses")
    lines.append("  CONN_COUNT=$(ss -tn state established '( sport >= 5201 and sport <= 5250 )' 2>/dev/null | tail -n +2 | wc -l)")
