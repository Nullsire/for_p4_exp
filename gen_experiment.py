...
def generate_receiver_script(args, config, filename="run_receiver.sh"):
    ...
    lines.append("}")
    lines.append("")
    lines.append("# Use a flag to prevent duplicate cleanup calls")
    lines.append("CLEANUP_DONE=0")
    lines.append("safe_cleanup() {")
    lines.append("  if [ $CLEANUP_DONE -eq 0 ]; then")
    lines.append("    CLEANUP_DONE=1")
    lines.append("    cleanup")
    lines.append("  fi")
    lines.append("  exit 0")
    lines.append("}")
    lines.append("trap safe_cleanup SIGINT SIGTERM EXIT")
    lines.append("")
    lines.append("# Wait and show connection status periodically")
    lines.append("echo 'Receiver is ready. Waiting for traffic...'")
    lines.append("echo \"You can monitor traffic with: ss -tn state established '( dport >= 5201 and dport <= 5250 )'\"")
    lines.append("echo ''")
    lines.append("")
    lines.append("# Show connection count every 10 seconds")
    lines.append("while true; do")
    lines.append("  # Count active iperf3 flows (not TCP connections)")
    lines.append("  # iperf3 creates 2 TCP connections per flow (control + data)")
    lines.append("  # So we count unique server-side ports (sport) in the 5201-5250 range")
    lines.append("  # Use awk to find the local address column and extract port after last colon")
    lines.append("  FLOW_COUNT=$(ss -tn state established '( sport >= 5201 and sport <= 5250 )' 2>/dev/null | awk 'NR>1 {for(i=1;i<=NF;i++){if($i ~ /^192\\.168\\.6\\.[0-9]+:52[0-5][0-9]$/){n=split($i,a,\":\"); print a[n]}}}' | sort -u | wc -l)")
    lines.append('  echo "[$(date ' + "'+%H:%M:%S')] Active iperf3 flows: $FLOW_COUNT\"")
