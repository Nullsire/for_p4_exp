...
def generate_receiver_script(args, config, filename="run_receiver.sh"):
    ...
    lines.append("  # So we count unique local ports in the 5201-5250 range")
    lines.append("  FLOW_COUNT=$(ss -tn state established '( sport >= 5201 and sport <= 5250 )' 2>/dev/null | awk 'NR>1 {split($4,a,\":\"); print a[2]}' | sort -u | wc -l)")
