...
def plot_single_metric_optimized(args):
    ...
    for flow_type, color in palette.items():
        ...
        for _, flow_data in type_subset.groupby('flow_id'):
            ...
            # Break segments at large time gaps to avoid horizontal lines across inactive periods
            # Calculate time differences between consecutive points
            time_values = valid_data['time_sec'].values
            metric_values = valid_data[metric_col].values
            
            # Find indices where time gap is too large (> 1 second)
            ...
            for gap_idx in gap_indices:
                end_idx = gap_idx + 1  # gap_idx is the index before the gap
                if end_idx > start_idx:
                    segment_points = np.column_stack((time_values[start_idx:end_idx], metric_values[start_idx:end_idx]))
                    segments.append(segment_points)
                start_idx = end_idx
            
            # Add the last segment
            if start_idx < len(time_values):
                segment_points = np.column_stack((time_values[start_idx:], metric_values[start_idx:]))
                segments.append(segment_points)
...
def visualize_tcp_metrics_optimized(csv_file, output_dir):
    ...
    usecols = ['timestamp_ns', 'flow_type', 'flow_id', 'srtt_us', 'cwnd_segs', 'rate_delivered_bps', 'total_retrans']
    ...
    df['rtt_ms'] = df['srtt_us'] / 1000.0
    
    # Pre-calculate delivery rate in Mbps (from bps)
    df['delivery_rate_mbps'] = df['rate_delivered_bps'] / 1e6
    ...
    tasks = [
        ...
        (df[['time_sec', 'flow_type', 'flow_id', 'cwnd_segs']].copy(), 'cwnd_segs', 'CWND (segments)', 'Congestion Window over Time (Full Resolution)', 'cwnd_over_time_opt.png', output_dir, palette, False),
        ...
        (df[['time_sec', 'flow_type', 'flow_id', 'total_retrans']].copy(), 'total_retrans', 'Cumulative Retransmits', 'Retransmits over Time (Full Resolution)', 'retransmits_over_time_opt.png', output_dir, palette, False)
    ]
