...
for j in $(seq 1 1); do
  ...
  iperf3 -c $DST_IP -p $PORT -t 480 -i 0.1 -C cubic --logfile /dev/null &
  echo "  Started Cubic flow $CUBIC_FLOW_ID on port $PORT"
done
# Add 1 Prague flows for 480s
for j in $(seq 1 1); do
  PRAGUE_FLOW_ID=$((PRAGUE_FLOW_ID + 1))
  PORT=$((5201 + 25 + $PRAGUE_FLOW_ID - 1))
  iperf3 -c $DST_IP -p $PORT -t 480 -i 0.1 -C prague --logfile /dev/null &
  echo "  Started Prague flow $PRAGUE_FLOW_ID on port $PORT"
done
...
for j in $(seq 1 1); do
  CUBIC_FLOW_ID=$((CUBIC_FLOW_ID + 1))
  PORT=$((5201 + $CUBIC_FLOW_ID - 1))
  iperf3 -c $DST_IP -p $PORT -t 360 -i 0.1 -C cubic --logfile /dev/null &
  echo "  Started Cubic flow $CUBIC_FLOW_ID on port $PORT"
done
...
for j in $(seq 1 1); do
  PRAGUE_FLOW_ID=$((PRAGUE_FLOW_ID + 1))
  PORT=$((5201 + 25 + $PRAGUE_FLOW_ID - 1))
  iperf3 -c $DST_IP -p $PORT -t 360 -i 0.1 -C prague --logfile /dev/null &
  echo "  Started Prague flow $PRAGUE_FLOW_ID on port $PORT"
done
...
for j in $(seq 1 8); do
  CUBIC_FLOW_ID=$((CUBIC_FLOW_ID + 1))
  PORT=$((5201 + $CUBIC_FLOW_ID - 1))
  iperf3 -c $DST_IP -p $PORT -t 240 -i 0.1 -C cubic --logfile /dev/null &
  echo "  Started Cubic flow $CUBIC_FLOW_ID on port $PORT"
done
...
for j in $(seq 1 8); do
  PRAGUE_FLOW_ID=$((PRAGUE_FLOW_ID + 1))
  PORT=$((5201 + 25 + $PRAGUE_FLOW_ID - 1))
  iperf3 -c $DST_IP -p $PORT -t 240 -i 0.1 -C prague --logfile /dev/null &
  echo "  Started Prague flow $PRAGUE_FLOW_ID on port $PORT"
done
...
for j in $(seq 1 15); do
  CUBIC_FLOW_ID=$((CUBIC_FLOW_ID + 1))
  PORT=$((5201 + $CUBIC_FLOW_ID - 1))
  iperf3 -c $DST_IP -p $PORT -t 120 -i 0.1 -C cubic --logfile /dev/null &
  echo "  Started Cubic flow $CUBIC_FLOW_ID on port $PORT"
done
...
for j in $(seq 1 15); do
  PRAGUE_FLOW_ID=$((PRAGUE_FLOW_ID + 1))
  PORT=$((5201 + 25 + $PRAGUE_FLOW_ID - 1))
  iperf3 -c $DST_IP -p $PORT -t 120 -i 0.1 -C prague --logfile /dev/null &
  echo "  Started Prague flow $PRAGUE_FLOW_ID on port $PORT"
done
